...
async def repair_one(
    ...
    async with sem:
        ...
        for host, source in candidates:
            ...
            same_brand = final_host is not None and host_is_self(host_norm, final_host)
            slug_in_host = host_matches_slug(host_norm, slug, name)
            links_to_ats = await page_links_to_ats(
                client, final_host or host_norm, slug, providers
            )
            ...
            if same_brand and links_to_ats and slug_in_host:
                ...
                if apex != host_norm:
                    ...
                    if apex_ok and apex_final and host_is_self(apex, apex_final):
                        host_norm = apex
                evidence.append(
                    f"{host_norm} ({source}): {why}; same-brand + links-to-ats"
                )
                return Proposal(slug, name, current, host_norm, "high", evidence)
            ...
            if from_ats and same_brand:
                ...
                if medium_fallback is None:
                    medium_fallback = Proposal(
                        slug, name, current, host_norm, "medium", list(evidence)
                    )
...
async def main() -> int:
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Rewrite the YAML in place for high-confidence proposals.",
    )
    parser.add_argument(
        "--include-medium",
        action="store_true",
        help="Also apply medium-confidence proposals (review the diff carefully).",
    )
    parser.add_argument("--concurrency", type=int, default=10)
    parser.add_argument("--only", nargs="*", help="Restrict to these slugs.")
    args = parser.parse_args()
    ...
    async with httpx.AsyncClient(
        timeout=TIMEOUT, headers=headers, http2=False
    ) as client:
        proposals = await asyncio.gather(
            *(repair_one(client, c, sem) for c in companies)
        )

    by_conf: dict[str, list[Proposal]] = {
        "high": [],
        "medium": [],
        "none": [],
        "skip": [],
    ...
    to_apply = {
        p.slug: p.new_domain
        for p in proposals
        if p.confidence in accept and p.new_domain
    }
    if not to_apply:
        print("No proposals to apply.", file=sys.stderr)
        return 0
